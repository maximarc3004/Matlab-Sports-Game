...
for i=3:length(sheets)
    for j = 1:height(data{1,i})
        if ~isempty(data{1,i}.Var1{j})
            ...
            palmares{q} = "("+string(data{1,i}.GoldOL(j))+"-"+string(data{1,i}.SilverOL(j))+"-"+string(data{1,i}.BronzeOL(j))+") ("+string(data{1,i}.GoldWC(j))+"-"+string(data{1,i}.SilverWC(j))+"-"+string(data{1,i}.BronzeWC(j))+")";
            q = q+1;
        end
    end
end

save("ListeAthletisme.mat", "athletes", "disciplines", "palmares");
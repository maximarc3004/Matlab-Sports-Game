...
while ~isempty(data{1,2}.Var1{j})
    annees{j} = data{1,2}.Var1{j};
    palmares{j} = string(data{1,2}.Var5(j))+" ("+string(data{1,2}.Var2(j))+"-"+string(data{1,2}.Var3(j))+"-"+string(data{1,2}.Var4(j))+")";
    j = j+1;
end

save("ListeBelgiqueCMAthletisme.mat", "annees", "palmares");
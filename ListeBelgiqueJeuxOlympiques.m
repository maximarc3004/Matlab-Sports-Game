...
j = 1;
while ~isempty(data{1,1}.Var1{j})
    annees{j} = data{1,1}.Var1{j};
    palmares{j} = string(data{1,1}.Var5(j))+" ("+string(data{1,1}.Var2(j))+"-"+string(data{1,1}.Var3(j))+"-"+string(data{1,1}.Var4(j))+")";
    j = j+1;
end

save("ListeBelgiqueJO.mat", "annees", "palmares");